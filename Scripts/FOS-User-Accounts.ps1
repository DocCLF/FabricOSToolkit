...
function FOS_User_Add {
    ...
    * The account name cannot be the same as an existing user account, a default role, a user-defined role, or a system role. 
    ...
    begin{
        Write-Debug -Message "Begin block |$(Get-Date)"
        Write-Debug -Message "$UserName,$SwitchIP,$FOS_UserName,$FOS_FID,$FOS_Role,$FOS_passwd"
    ...
    process{
        ...
        Write-Debug -Message "ssh $UserName $($SwitchIP) userconfig --add $FOS_UserName -l $FOS_FID -r $FOS_Role -p $FOS_passwd "
    }
    end{
        Write-Debug -Message "End block |$(Get-Date)"
        $FOS_user_add
...
function FOS_User_Modify {
    ...
    process{
        Write-Debug -Message "Process block |$(Get-Date)"
        switch ($FOS_Operand) {
            "change" { $FOS_user_add = ssh $UserName@$($SwitchIP) "userconfig --$FOS_Operand $FOS_UserName -l $FOS_FID -r $FOS_Role -p $FOS_passwd" }
